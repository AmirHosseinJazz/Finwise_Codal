...
IC_license=['lblForthPlaning','lblFourth_FirstchkBox','lblFourth_SecondchkBox','lblFourth_ThirdchkBox','lblFourth_FourthchkBox','lblFourth_FifthchkBox','lblFourth_SixthchkBox','lblFourth_SeventhchkBox','lblFourth_EighthchkBox','lblFourthTransparenciesImpotant']
false_Labels=['lblForthPlaning','lblFourth_FirstchkBox','lblFourth_SecondchkBox','lblFourth_ThirdchkBox','lblFourth_FourthchkBox','lblFourth_FifthchkBox','lblFourth_SixthchkBox','lblFourth_SeventhchkBox','lblFourth_EighthchkBox','lblFourthTransparenciesImpotant']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'FB2')
...
def get_unconverted():
    try:
        ...
        df = psql.read_sql("""
            ...
            select * FROM codalraw."allrawReports" where ("LetterCode"='ف ب-۲' OR "LetterCode"='ب-۲' )and "HtmlUrl"!='' and "TracingNo" not in (select "report_ID" from codalraw."BourseClarificationConverted")
...
def InsertExemptNonCompli(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from  codalreports."BourseClarification_ExemptionRegardingNonCompliance" where "report_id"=%(report_id)s ) THEN
                        INSERT INTO  codalreports."BourseClarification_ExemptionRegardingNonCompliance"(
                        firm, report_id, "PublishDate", "Issues", "Reasons", "Conditions", "Source")
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,
                        %(DateOf)s,%(lblFourthTransparenciesImpotant)s,%(Reasons)s,%(Conditions)s,
                        %(lblForthPlaning)s
                        );
                    END IF;
                END
...
def handle_all(driver,df):
    ...
    for i in IC_license:
        ...
        if (i not in DFIC.columns)&('lbl' in i):
            DFIC[i]=''
    DFIC['Reasons']=DFIC['lblFourth_FirstchkBox']+' - '+DFIC['lblFourth_SecondchkBox']+' - '+DFIC['lblFourth_ThirdchkBox']+' - '+DFIC['lblFourth_FourthchkBox']+' - '+DFIC['lblFourth_FifthchkBox']
    DFIC['Conditions']=DFIC['lblFourth_SixthchkBox']+' - '+DFIC['lblFourth_SeventhchkBox']+' - '+DFIC['lblFourth_SeventhchkBox']+' - '        
    for index,row in DFIC.iterrows():
        InsertExemptNonCompli(row)                  
def RUN(driver):
    df=get_unconverted()
    if not df.empty:
        ...
        print('All FB1_NonCompliance DONE')            