...
IC_license=['lblThirdTransparenciesImpotant','lblThirdLetterNo','lblThirdLetterDate','lblThird_FourthchkBox','lblThird_SecondchkBox','lblThird_FirstchkBox','lblThird_ThirdchkBox']
false_Labels=['lblThirdTransparenciesImpotant','lblThirdLetterNo','lblThirdLetterDate','lblThird_FourthchkBox','lblThird_SecondchkBox','lblThird_FirstchkBox','lblThird_ThirdchkBox']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'FB1')
...
def get_unconverted():
    try:
        ...
        select * FROM codalraw."allrawReports" where ("LetterCode"='ف ب-۱' OR "LetterCode"='ب-۱' )and "HtmlUrl"!='' and "TracingNo" not in (select "report_ID" from codalraw."BourseClarificationConverted")
...
def InsertNonComp(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from  codalreports."BourseClarification_NoRespone_NonCompliance" where "report_id"=%(report_id)s ) THEN
                        INSERT INTO  codalreports."BourseClarification_NoRespone_NonCompliance"(
                        firm, report_id, "PublishDate", "Issue", "LetterCode", "LetterDate", 
                        "Clause1", "Clause2", "Problems", "DelayinPublication")
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,
                        %(DateOf)s,%(lblThirdTransparenciesImpotant)s,%(lblThirdLetterNo)s,%(lblThirdLetterDate)s,
                        %(lblThird_FirstchkBox)s,
                        %(lblThird_SecondchkBox)s,%(lblThird_ThirdchkBox)s,%(lblThird_FourthchkBox)s
                        );
                    END IF;
                END
            $$ 
