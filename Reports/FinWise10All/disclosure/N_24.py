...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'Disclosure_N24')
...
def get_unconverted():
    try:
        ...
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۲۴' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."DisclosureConverted")
...
def InsertOppArticle19(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."Disclosure_RequestingTimeToComplyWithArticle19" where "report_id"=%(report_id)s) THEN
                        INSERT INTO codalreports."Disclosure_RequestingTimeToComplyWithArticle19"(
                        firm, report_id, "Correction", "CorrectionDetails", "PublishDate", "RequestToDate")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s
                        ,%(DateOf)s,%(lblTwentySecondFromDate)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert Disclosure_Rumor", error)
                log_it('Failed to Insert Disclosure_Rumor -N24'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    # DFIC['Source']=DFIC['lblFirstPlaning']+DFIC['lblFirstTransparenciesSource']
    # DFIC['Desc']=DFIC['lblFirstTransparenciesSourceDescription']+DFIC['lblFirstDescription1']
    for i in IC_license:
    ...
    for index,row in DFIC.iterrows():
        InsertOppArticle19(row)                    
def RUN(driver):
    df=get_unconverted()
    if not df.empty:
        handle_all(driver,df)
    else:
        print('All Article 19_Disclosure DONE')