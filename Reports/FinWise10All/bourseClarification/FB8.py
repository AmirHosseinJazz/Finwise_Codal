...
IC_license=['lblTwelfthTransparenciesImpotant','lblTwelfthPlaning','lblTwelfthLetterNo','lblTwelfthLetterDate']

false_Labels=['lblTwelfthTransparenciesImpotant','lblTwelfthPlaning','lblTwelfthLetterNo','lblTwelfthLetterDate']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'FB8')
...
def get_unconverted():
    try:
        ...
        df = psql.read_sql("""
         ...
         select * FROM codalraw."allrawReports" where ("LetterCode"='ف ب-۸' OR "LetterCode"='ب-۸' )and "HtmlUrl"!='' and "TracingNo" not in (select "report_ID" from codalraw."BourseClarificationConverted")
...
def InsertCCallNoResp(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from  codalreports."BourseClarification_NoRespone_ConfrenceCall" where "report_id"=%(report_id)s ) THEN
                        INSERT INTO  codalreports."BourseClarification_NoRespone_ConfrenceCall"(
                        ...
                        %(DateOf)s,%(lblTwelfthTransparenciesImpotant)s,%(lblTwelfthPlaning)s,%(lblTwelfthLetterNo)s,
                        %(lblTwelfthLetterDate)s
                        );
                    END IF;
                END
            $$ 
