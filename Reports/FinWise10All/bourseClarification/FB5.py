...
IC_license=['lblSeventh_ThirdchkBox','lblSeventh_FourthchkBox','lblSeventhPlaning','lblSeventhLetterNo','lblSeventhLetterDate','lblSeventhOpportunity','lblSeventhTransparenciesObserveInstructions','lblSeventhExtended','lblSeventh_SixthchkBox','lblSeventh_SevenchkBox'
           ,'lblSeventh_FirstchkBox','lblSeventh_SecondchkBox'
           
           ]
false_Labels=['lblSeventh_ThirdchkBox','lblSeventh_FourthchkBox','lblSeventhPlaning','lblSeventhLetterNo','lblSeventhLetterDate','lblSeventhOpportunity','lblSeventhTransparenciesObserveInstructions','lblSeventhExtended','lblSeventh_SixthchkBox','lblSeventh_SevenchkBox'
           ,'lblSeventh_FirstchkBox','lblSeventh_SecondchkBox'
           ]
...
def get_unconverted():
    try:
        ...
        select * FROM codalraw."allrawReports" where ("LetterCode"='ف ب-۵' OR "LetterCode"='ب-۵' )and "HtmlUrl"!='' and "TracingNo" not in (select "report_ID" from codalraw."BourseClarificationConverted")
...
def InsertExemptAfterOpp(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from  codalreports."BourseClarification_ExemptionAfterExtendedOpportunity" where "report_id"=%(report_id)s ) THEN
                        INSERT INTO  codalreports."BourseClarification_ExemptionAfterExtendedOpportunity"(
                        firm, report_id, "PublishDate", "Source", "LetterCode",
                        "LetterDate", "Opportunity", "ExtendedTime",
                        "Instruction", "Reasons", "Conditions")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(DateOf)s,%(lblSeventhPlaning)s,%(lblSeventhLetterNo)s,%(lblSeventhLetterDate)s,
                        %(lblSeventhOpportunity)s,%(lblSeventhExtended)s,%(lblSeventhTransparenciesObserveInstructions)s,
                        %(Reasons)s,%(Conditions)s
                        );
                    END IF;
                END
            $$ 
...
def handle_all(driver,df):
    ...
    for i in IC_license:
        ...
        if (i not in DFIC.columns)&('lbl' in i):
            DFIC[i]=''
    DFIC['Reasons']=DFIC['lblSeventh_ThirdchkBox']+' - '+DFIC['lblSeventh_FourthchkBox']+' - '+DFIC['lblSeventh_FirstchkBox']+' - '+DFIC['lblSeventh_SecondchkBox']
    DFIC['Conditions']=DFIC['lblSeventh_SevenchkBox']+' - '+DFIC['lblSeventh_SixthchkBox']
    for index,row in DFIC.iterrows():
        InsertExemptAfterOpp(row)                   
def RUN(driver):
    df=get_unconverted()
    if not df.empty:
