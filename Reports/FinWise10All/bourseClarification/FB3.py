...
IC_license=['lblFifthTransparenciesImpotant','lblFifth_FirstchkBox','lblFifth_SecondchkBox','lblFifthPlaning','lblFifthOpportunity','lblFifthTransparenciesObserveInstructions','lblFifthTodate']
false_Labels=['lblFifthTransparenciesImpotant','lblFifth_FirstchkBox','lblFifth_SecondchkBox','lblFifthPlaning','lblFifthOpportunity','lblFifthTransparenciesObserveInstructions','lblFifthTodate']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'FB3')
...
def get_unconverted():
    try:
        ...
        df = psql.read_sql("""
               ...
               select * FROM codalraw."allrawReports" where ("LetterCode"='ف ب-۳' OR "LetterCode"='ب-۳' )and "HtmlUrl"!='' and "TracingNo" not in (select "report_ID" from codalraw."BourseClarificationConverted")
...
def InsertExtendOpport(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from  codalreports."BourseClarification_ExtendingOpportunity" where "report_id"=%(report_id)s ) THEN
                        INSERT INTO  codalreports."BourseClarification_ExtendingOpportunity"(
                        firm, report_id, "PublishDate", 
                        "Opportunity", "OpportunityToDate", 
                        "Issue", "Reason", "Source")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(DateOf)s,%(Opportuinty)s,%(lblFifthTodate)s,%(lblFifthTransparenciesImpotant)s,
                        %(Reasons)s,%(lblFifthPlaning)s
                        );
                    END IF;
                END
            $$ 
...
def handle_all(driver,df):
    ...
    for i in IC_license:
        ...
        if (i not in DFIC.columns)&('lbl' in i):
            DFIC[i]=''
    DFIC['Reasons']=DFIC['lblFifth_FirstchkBox']+' - '+DFIC['lblFifth_SecondchkBox']
    DFIC['Opportuinty']=DFIC['lblFifthOpportunity']+' - '+DFIC['lblFifthTransparenciesObserveInstructions']
    for index,row in DFIC.iterrows():
        InsertExtendOpport(row)           
def RUN(driver):
    df=get_unconverted()
    if not df.empty:
