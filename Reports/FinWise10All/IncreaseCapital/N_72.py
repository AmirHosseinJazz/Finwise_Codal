...
IC_license=['lblLetterDate','lblBoardMemberDate']
false_Labels=['lblLetterDate','lblBoardMemberDate']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N72')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۷۲' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertIC_Revoke(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from   codalreports."IncreaseCapital_CancelBoardInvitation" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."IncreaseCapital_CancelBoardInvitation"(
                        firm, report_id, "Correction", "CorrectionDetails", "InvitationLetterDate", "AssemblyDate")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s,
                        %(lblLetterDate)s,%(lblBoardMemberDate)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                ...
                log_it('Failed to Insert RegNotice N72 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
