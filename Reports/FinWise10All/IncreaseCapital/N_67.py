...
IC_Posculate=['txbEndDate','txbStartDate','lblLicenseDesc',
              'lblLastExtraAssembly','lblPreviousCapital','lblNewCapital','txbCashIncoming','txbRetaindedEarning',
                    'txbReserves','txbRevaluationSurplus','txbSarfSaham','lblShareValue','txbDate']
false_Labels=['lblYearEndToDate','lblShareValue','lblLicenseDesc','lblLastExtraAssembly',
              'lblLastBoardMemberInvitation','lblPreviousCapital','lblNewCapital']
...
def InsertError(CodalRaw_ID):
    try:
        ...
        connection.commit()
        log_it("N_67_Error "+str(CodalRaw_ID))
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N67')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۶۷' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertIC_RegNotice(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from  codalreports."IncreaseCapital_RegisterationNotice" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."IncreaseCapital_RegisterationNotice"(
                        firm, report_id, "Correction", "CorrectionDetails", "RegistrationDate", "LicenseRef",
                        "CashIncoming", "Reserved", "RetainedEarning", "RevaluationSurplus", "SarfSaham")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s,
                        %(txbDate)s,%(lblLicenseDesc)s,
                        %(txbCashIncoming)s,%(txbReserves)s,%(txbRetaindedEarning)s,%(txbRevaluationSurplus)s,
                        %(txbSarfSaham)s
                        );
                    END IF;
                END
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                ...
                log_it('Failed to Insert RegNotice N67 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
