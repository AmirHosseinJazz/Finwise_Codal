...
IC_license=['lblLicenseNo','lblLicenseDate','lblForTime','lblToDate']
false_Labels=['lblLicenseNo','lblLicenseDate','lblForTime','lblToDate']
        ...
        postgres_insert_query = """
          INSERT INTO public._log(
            date, action,source)
                VALUES (%s, %s,%s);
        """
        record_to_insert = (str(datetime.datetime.now()),text,'N63')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۶۳' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertIC_deadline(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."IncreaseCapital_DeadlineExtension" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."IncreaseCapital_DeadlineExtension"(
                        firm, report_id, "Correction", "CorrectionDetails", 
                        "LicenseRef", "LicenseDate", "ExtenstionLenght", 
                        "NewDeadline")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s,
                        %(lblLicenseNo)s,%(lblLicenseDate)s,
                        %(lblForTime)s,%(lblToDate)s
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert Deadline ", error)
                log_it('Failed to Insert Deadline N63 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
