...
IC_Assembly_labels=['lblYearEndToDate','lblLicenseDesc','txbPreviousCapital','txbNewCapital',
                'txbSessionDay','txbSessionDate',
                'lblCashIncomingCaption1','txbCashIncoming','lblCashIncomingCaption2',
                'lblReservesCaption1','txbReserves','lblReservesCaption2',
                'lblRetaindedEarningCaption1','txbRetaindedEarning','lblRetaindedEarningCaption2',
                'lblRevaluationSurplusCaption1','txbRevaluationSurplus','lblRevaluationSurplusCaption2',
                'lblSarfSahamCaption1','txbSarfSaham','lblSarfSahamCaption2','txbLastExtraAssembly','lblLastExtraAssembly'
                
                ]
false_Labels=['lblYearEndToDate','lblLicenseDesc','lblLastExtraAssembly'] 
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N71')
...
def insertICATime(row):
    try:
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert ICATIME ", error)
                log_it('Failed to Insert ICATIME N71-'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    for i in IC_Assembly_labels:
        ...
        if (i not in DFIC.columns)&('lbl' in i):
            DFIC[i]=False
    DFIC['NewCapital']=DFIC['txbNewCapital'].apply(get_true_value)
    DFIC['previousCapital']=DFIC['txbPreviousCapital'].apply(get_true_value) 
