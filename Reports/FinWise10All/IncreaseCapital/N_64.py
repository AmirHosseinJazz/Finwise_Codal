...
IC_Priority=['txbEndDate','txbStartDate','lblLicenseDesc','lblLastExtraAssembly','lblPreviousCapital','lblNewCapital']
false_Labels=['lblYearEndToDate','lblLicenseDesc','lblLastExtraAssembly','lblLastBoardMemberInvitation','lblPreviousCapital','lblNewCapital']
...
def InsertError(CodalRaw_ID):
    try:
        ...
        connection.commit()
        log_it("N_63_Error "+str(CodalRaw_ID))
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N64')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۶۴' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertIC_PriorityTime(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."IncreaseCapital_PriorityTime" where "report_id"=%(report_id)s) THEN
                        INSERT INTO codalreports."IncreaseCapital_PriorityTime"(
                        firm, report_id, "Correction",
                        "CorrectionDetails", "Revival",
                        "endDate", "startDate", "LicenseRef",
                        ...
                        %(txbStartDate)s,%(lblLicenseDesc)s,%(lblLastExtraAssembly)s,
                        %(previousCapital)s,%(NewCapital)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert IC_PriorityTime ", error)
                log_it('Failed to Insert IC_PriorityTime N64-'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
