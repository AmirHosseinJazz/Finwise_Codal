...
IC_license=['lblAuditorName','lblSessionDate','lblFromAmont','lblToAmount','lblCapitalIncreaseType',
            'lblSubject']
false_Labels=['lblAuditorName','lblSessionDate','lblFromAmont','lblToAmount','lblCapitalIncreaseType','lblSubject']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'Inspector_N61')
...
def get_unconverted():
    try:
        ...
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۶۱' and "HtmlUrl"!='' and "Available"=True and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
...
def insertIC_Auditor(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."IncreaseCapital_AuditInfo" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."IncreaseCapital_AuditInfo"(
                        firm, report_id, "Correction",
                        "CorrectionDetails", "SessionDate",
                        "AuditorName", "FromAmount", "ToAmount",
                        "CapitalChangeType", "Subject")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s,
                        %(lblSessionDate)s,%(lblAuditorName)s,
                        %(lblFromAmont)s,%(lblToAmount)s,
                        %(lblCapitalIncreaseType)s,%(lblSubject)s
