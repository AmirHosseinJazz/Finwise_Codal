...
import re
IC_Posculate=['txbEndDate','txbStartDate','lblLicenseDesc','txbPriorityTimeShareCount',
              'lblLastExtraAssembly','lblPreviousCapital','lblNewCapital','txbPostulateShareCount',
            'lblShareValue']

false_Labels=['lblYearEndToDate','lblShareValue','lblLicenseDesc','lblLastExtraAssembly','lblLastBoardMemberInvitation','lblPreviousCapital','lblNewCapital']
db_username="Jay"
db_pass="Mehrad1"
db_host="127.0.0.1"
db_port="5432"
...
def InsertError(CodalRaw_ID):
    try:
        ...
        connection.commit() 
        log_it("N_65_Error "+str(CodalRaw_ID))
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N65')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۶۵' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertIC_PostulateDics(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."IncreaseCapital_PostulateDiscussion" where "report_id"=%(report_id)s) THEN
                        INSERT INTO codalreports."IncreaseCapital_PostulateDiscussion"(
                        firm, report_id, "Correction", "CorrectionDetails", 
                        "Revival", "endDate", "startDate", "LicenseRef", "PriorityTimeshareCount",
                        "LastExtraAssembly", "PreviousCapital", 
                        "NewCapital", "shareValue", "PostulateShareCount")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s,
                        %(Revival)s,%(txbEndDate)s,
                        %(txbStartDate)s,%(lblLicenseDesc)s,%(prioCount)s,%(lblLastExtraAssembly)s,
                        %(previousCapital)s,%(NewCapital)s,%(sharevalue)s,%(postulateCount)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert IC_PostulateDics ", error)
                #log_it('Failed to Insert IC_PostulateDics N65 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    for i in IC_Posculate:
        ...
        if (i not in DFIC.columns)&('lbl' in i):
            DFIC[i]=False
    DFIC['NewCapital']=DFIC['lblNewCapital'].apply(get_true_value)
    DFIC['previousCapital']=DFIC['lblPreviousCapital'].apply(get_true_value)         
    DFIC['sharevalue']=DFIC['lblShareValue'].apply(get_true_value)   
    DFIC['prioCount']=DFIC['txbPriorityTimeShareCount'].apply(get_true_value)   
    DFIC['postulateCount']=DFIC['txbPostulateShareCount'].apply(get_true_value)   
    for index,row in DFIC.iterrows():
        insertIC_PostulateDics(row)        
def RUN(driver):
    df=get_unconverted()
