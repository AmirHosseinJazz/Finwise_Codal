...
IC_license=['lblSessionDate','lblFromAmount','lblToAmount','lblCapitalChangeType',
            'lblSubject','lblStep','lblFromAmountBoardMember','lblToAmountBoardMember',
            'lblCapitalChangeTypeBoardMember','lblSubjectBoardMember'
           ]
false_Labels=['lblSessionDate','lblFromAmount','lblToAmount','lblCapitalChangeType',
            'lblSubject','lblStep','lblFromAmountBoardMember','lblToAmountBoardMember',
            'lblCapitalChangeTypeBoardMember','lblSubjectBoardMember'
           ]
...
def InsertError(CodalRaw_ID):
    try:
        ...
        connection.commit() 
        log_it("N_70Error "+str(CodalRaw_ID))
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N70')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۷۰' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertIC_MultiStep(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."IncreaseCapital_MultiStepBoardMember" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."IncreaseCapital_MultiStepBoardMember"(
                        firm, report_id, "Correction", "CorrectionDetails",
                        "SessionDate", "FromAmount_First", "ToAmount_First",
                        "CapitalChangeType_First", "Subject_First", "Step", 
                        "FromAmount_Step", "ToAmount_Step", 
                        "CapitalChangeType_Step", "Subject_Step")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s,
                        %(lblSessionDate)s,%(lblFromAmount)s,
                        %(lblToAmount)s,%(lblCapitalChangeType)s,
                        %(lblSubject)s,%(lblStep)s,
                        %(lblFromAmountBoardMember)s,%(lblToAmountBoardMember)s,%(lblCapitalChangeTypeBoardMember)s,
                        %(lblSubjectBoardMember)s
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert Multistep ", error)
                log_it('Failed to Insert Multistep N70-'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
