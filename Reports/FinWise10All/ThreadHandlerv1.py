...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۱۰' and "HtmlUrl"!='' and "Available"=True and "TracingNo" not in (select "report_ID" from codalraw."SheetsConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insert_titlebox(titles):
    try:
        ...
        pouery = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from  public."Entity" where "ticker"=%(ticker)s) THEN
         ...
         DO 
            ...
            BEGIN
                IF NOT EXISTS (select from statement."Titles" where "report_id"=%(report_id)s) THEN
                    ...
                    %(unregistered)s, (select "ID" from "Entity" where ticker=%(ticker)s), %(industryCode)s, %(header_PeriodLength)s,
