...
IC_license=['lblSixteenth2_Firstchk','lblSixteenth1_Firstchk','lblSixteenth1_SecondchkBox','lblSixteenth2_SecondchkBox','lblSixteenth1_ThirdchkBox','lblSixteenth2_ThirdchkBox','lblSixteenthToDate']
false_Labels=['lblSixteenth2_Firstchk','lblSixteenth1_Firstchk','lblSixteenth1_SecondchkBox','lblSixteenth2_SecondchkBox','lblSixteenthToDate','lblSixteenth1_ThirdchkBox','lblSixteenth2_ThirdchkBox']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'S12')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='س-۱۲' and "HtmlUrl"!='' and "Available"=True and "TracingNo" not in (select "report_ID" from codalraw."orgClarificationConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def InsertExempt(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from  codalreports."OrgClarification_ExemptionRequest" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."OrgClarification_ExemptionRequest"(
                        firm, report_id, "Subject", "PublishTime", "toDate", "Reason")
                        VALUES ( (select "ID" from "Entity" where ticker=%(Nemad)s), %(report_id)s,
                        %(Subject)s,%(DateOf)s,
                        %(lblSixteenthToDate)s,
                        %(Reason)s
                        );
                    END IF;
                END
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert OrgClar_S12 ", error)
                log_it('Failed to Insert OrgClar_S12 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    for index,row in df.iterrows():
        try:
            ...
            if UpdateError(driver,CodalRaw_ID):
                ...
                Nemad=''
                Subject='ExtendingExemptionRequest'
    ...
    for index,row in DFIC.iterrows():
        if (row['lblSixteenth2_ThirdchkBox']!='' )or (row['lblSixteenth1_ThirdchkBox']!=''):
            DFIC.at[index,'Reason']='وضعیت شفافیت اطلاعاتی ناشر'
        if (row['lblSixteenth2_SecondchkBox']!='' )or (row['lblSixteenth1_SecondchkBox']!=''):
            DFIC.at[index,'Reason']='در خصوص ظن تقلب یا دستکاری در اطلاعات منتشره یا صورت های مالی ناشر'
        if (row['lblSixteenth2_Firstchk']!='' )or (row['lblSixteenth1_Firstchk']!=''):
            DFIC.at[index,'Reason']='علت عدم افشای بموقع اطلاعات با اهمیت'
