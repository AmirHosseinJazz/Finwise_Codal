...
IC_license=['lblTwentyFourTransparenciesPlaning1','lblTwentyFourTransparenciesPlaning2']

false_Labels=['lblTwentyFourTransparenciesPlaning1','lblTwentyFourTransparenciesPlaning2']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'S15')
...
def get_unconverted():
    try:
        ...
        select * FROM codalraw."allrawReports" where "LetterCode"='س-۱۵' and "HtmlUrl"!='' and "Available"=True and "TracingNo" not in (select "report_ID" from codalraw."orgClarificationConverted")
...
def InsertExiting(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."OrgClarification_ExitingFromPrecaution" where "report_id"=%(report_id)s) THEN
                        INSERT INTO codalreports."OrgClarification_ExitingFromPrecaution"(
                        firm, report_id, "Subject", "PublishTime", "Desc")
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert OrgClar_S15 ", error)
                log_it('Failed to Insert OrgClar_S15 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    for index,row in DFIC.iterrows():
        DFIC.at[index,'DESC']=row['lblTwentyFourTransparenciesPlaning1']+str(row['lblTwentyFourTransparenciesPlaning1'])
    for index,row in DFIC.iterrows():
        InsertExiting(row)      
def RUN(driver):
    df=get_unconverted()
