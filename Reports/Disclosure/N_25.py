...
IC_license=['lblTwentyFive','lblTwentyFiveDescription']

false_Labels=['lblTwentyFive','lblTwentyFiveDescription']
        ...
        postgres_insert_query = """
          ...
          INSERT INTO public._log(
            date, action,source)
                VALUES (%s, %s,%s);
        """
        record_to_insert = (str(datetime.datetime.now()),text,'Disclosure_N25')
...
def insertArticle141(row):
    try:
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert Disclosure_insertArticle141", error)
                log_it('Failed to Insert Disclosure_insertArticle141 N25-'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    for index,row in DFIC.iterrows():
        insertArticle141(row)                    
def RUN(driver):
    df=get_unconverted()
    if not df.empty:
        handle_all(driver,df)
    else:
        print('All Exit 141 DONE')                      