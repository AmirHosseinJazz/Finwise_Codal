...
IC_license=[
    'lblThirtyThirdTransparenciesPlaning',
    'lblThirtyThirdTracingNoDescription',
    'lblThirtyThirdDescription1',
    'lblThirtyThirdLetterNo',
    'lblThirtyThirdLetterDate',
    'lblThirtyThirdImpotant',
    'lblThirtyThirdTransparenciesSourceDescription',
    'lblThirtyThirdTransparenciesDescription'
]
        ...
        postgres_insert_query = """
          ...
          INSERT INTO public._log(
            date, action,source)
                VALUES (%s, %s,%s);
        """
        record_to_insert = (str(datetime.datetime.now()),text,'Disclosure_N26')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۲۶' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."DisclosureConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def InsertFurtherInfo(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."Disclosure_FurtherInformationOnPublishedDocuments" where "report_id"=%(report_id)s) THEN
                        INSERT INTO codalreports."Disclosure_FurtherInformationOnPublishedDocuments"(
                        firm, report_id, "Correction", "correctionDetails",
                        "PublishDate", "Source", "Issue"
                        , "Action", "LetterDate", "LetterNo")
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s
                        ,%(DateOf)s,%(lblThirtyThirdImpotant)s,
                        %(Issues)s,%(Actions)s,%(lblThirtyThirdLetterDate)s,%(lblThirtyThirdLetterNo)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert Disclosure_InsertFurtherInfo", error)
                log_it('Failed to Insert Disclosure_InsertFurtherInfo N26-'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    DFIC['Actions']=DFIC['lblThirtyThirdDescription1']+DFIC['lblThirtyThirdTransparenciesDescription']
    DFIC['Issues']=DFIC['lblThirtyThirdTransparenciesSourceDescription']+DFIC['lblThirtyThirdTransparenciesPlaning']
    for i in IC_license:
    ...
    for index,row in DFIC.iterrows():
        InsertFurtherInfo(row)                    
def RUN(driver):
    df=get_unconverted()
    if not df.empty:
        handle_all(driver,df)
    else:
        print('All InsertFurtherInfo DONE')                     