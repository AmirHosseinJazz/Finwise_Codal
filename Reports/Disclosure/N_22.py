...
IC_license=['lblTwentiethTransparenciesSourceDescription','lblTwentiethToDate','lblTwentiethCompanyType3','lblTwentiethCompanyType2'
            ,'lblTwentiethCompanyType3','lblTwentiethDescription','lblTwentiethCompanyType1','lblTwentiethFromDate']

false_Labels=['lblTwentiethTransparenciesSourceDescription','lblTwentiethToDate','lblTwentiethCompanyType3','lblTwentiethCompanyType2'
            ,'lblTwentiethCompanyType3','lblTwentiethDescription','lblTwentiethCompanyType1','lblTwentiethFromDate']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'Disclosure_N22')
...
def get_unconverted():
    try:
        ...
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۲۲' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."DisclosureConverted")
...
def insertDISClo_price(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from   codalreports."Disclosure_PriceVolatility" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."Disclosure_PriceVolatility"(
                        firm, report_id, "Correction", 
                        "CorrectionDetails", "PublishDate",
                        "Issue", "Description", "LastReportDate")
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,%(correction)s
                        ,%(Details)s,%(DateOf)s,
                        %(Issues)s,%(DESC)s,
                        %(lastRepDate)s
                        );
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert Disclosure_PriceVol ", error)
                log_it('Failed to Insert Disclosure_PriceVol - N22'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    DFIC['Issues']=DFIC['lblTwentiethCompanyType3']+DFIC['lblTwentiethCompanyType2']
    DFIC['DESC']=DFIC['lblTwentiethTransparenciesSourceDescription']+DFIC['lblTwentiethDescription']
    DFIC['lastRepDate']=DFIC['lblTwentiethToDate']+DFIC['lblTwentiethFromDate']
    for i in IC_license:
    ...
    for index,row in DFIC.iterrows():
        insertDISClo_price(row)        
def RUN(driver):
    df=get_unconverted()
    if not df.empty:
        handle_all(driver,df)
    else:
        print('All Price Vol Efsha DONE')              