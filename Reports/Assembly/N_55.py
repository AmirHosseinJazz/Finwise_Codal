...
def InsertError(CodalRaw_ID):
    try:
        ...
        log_it("N_55_Error_"+str(CodalRaw_ID))
        connection.commit() 
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N_55')
...
def get_unconverted():
    try:
        ...
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۵۵' and "HtmlUrl"!='' and "Available"=True and "TracingNo" not in (select "report_ID" from codalraw."AssemblyConverted")
    ...
    try:
        ...
        postgres_insert_query = """
         DO 
            $$
            BEGIN
                IF NOT EXISTS (select from codalreports."Assembly_GeneralExtra" where "report_id"=%(report_id)s) THEN
                    INSERT INTO codalreports."Assembly_GeneralExtra"(
        ...
        postgres_insert_query_cheif = """
         DO 
            $$
            BEGIN
                IF NOT EXISTS (select from codalreports."Assembly_GeneralExtra_AssemblyChief" where "SummaryID"=(Select "ID" from codalreports."Assembly_GeneralExtra" where "report_id"=%(report_id)s) and "Position"=%(Position)s ) THEN
                    INSERT INTO codalreports."Assembly_GeneralExtra_AssemblyChief"(
                    "SummaryID", "Name", "Position")
                    VALUES ( (select "ID" from codalreports."Assembly_GeneralExtra" where report_id=%(report_id)s),%(Name)s,%(Position)s
        ...
        postgres_insert_query_board = """
         DO 
            $$
            BEGIN
                IF NOT EXISTS (select from codalreports."Assembly_GeneralExtra_Board" where "SummaryID"=(Select "ID" from codalreports."Assembly_GeneralExtra" where "report_id"=%(report_id)s) and "FullName"=%(FullName)s and "SSID"=%(SSID)s ) THEN
                    INSERT INTO codalreports."Assembly_GeneralExtra_Board"(
                    "SummaryID", "FullName", "SSID", "typeOfCompany", "Membership", "AgentName", "AgentSSID", "Position", "Duty", "Degree")
                    VALUES ( (select "ID" from codalreports."Assembly_GeneralExtra" where report_id=%(report_id)s),
        ...
        postgres_insert_query_ceo = """
         DO 
            $$
            BEGIN
                IF NOT EXISTS (select from codalreports."Assembly_GeneralExtra_CEO" where "SummaryID"=(Select "ID" from codalreports."Assembly_GeneralExtra" where "report_id"=%(report_id)s) and "FullName"=%(FullName)s and "SSID"=%(SSID)s ) THEN
                    INSERT INTO codalreports."Assembly_GeneralExtra_CEO"(
        ...
        postgres_insert_query_newBoard = """
         DO 
            $$
            BEGIN
                IF NOT EXISTS (select from codalreports."Assembly_GeneralExtra_NewBoard" where "SummaryID"=(Select "ID" from codalreports."Assembly_GeneralExtra" where "report_id"=%(report_id)s) and "Name"=%(Name)s) THEN
                    INSERT INTO codalreports."Assembly_GeneralExtra_NewBoard"(
        ...
        postgres_insert_query_present= """
         DO 
            $$
            BEGIN
                IF NOT EXISTS (select from codalreports."Assembly_GeneralExtra_PresentShareHolders" where "SummaryID"=(Select "ID" from codalreports."Assembly_GeneralExtra" where "report_id"=%(report_id)s) and "Shareholders"=%(Shareholders)s  ) THEN
                    INSERT INTO codalreports."Assembly_GeneralExtra_PresentShareHolders"(
        ...
        postgres_insert_query_statement= """
         DO 
            $$
            BEGIN
                IF NOT EXISTS (select from codalreports."Assembly_GeneralExtra_Statement" where "SummaryID"=(Select "ID" from codalreports."Assembly_GeneralExtra" where "report_id"=%(report_id)s) and "Title"=%(Desc)s  ) THEN
                    INSERT INTO codalreports."Assembly_GeneralExtra_Statement"(
        ...
        postgres_insert_query_wage= """
         DO 
            $$
            BEGIN
                IF NOT EXISTS (select from codalreports."Assembly_GeneralExtra_WageGift" where "SummaryID"=(Select "ID" from codalreports."Assembly_GeneralExtra" where "report_id"=%(report_id)s) and "Title"=%(Desc)s  ) THEN
                    INSERT INTO codalreports."Assembly_GeneralExtra_WageGift"(
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert GeneralExtra ", error)
                log_it('Failed to Insert GeneralExtra -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()             
...
def handleAllGeneralExtra(driver,df):
    listofDicts=[]
    ...
    for index,row in DFIC.iterrows():
        insert_generalextra(row)

def RUN(driver):
    df=get_unconverted()
    if not df.empty:
        handleAllGeneralExtra(driver,df)
    else:
        print('No New GeneralExtra_ N55')