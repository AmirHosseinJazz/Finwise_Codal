...
def update_Empty(Report_ID,TypeOf):
    print('initaite Empty..'+str(Report_ID))
    try:
        ...
        if TypeOf=="ISCONS":
            ...
            UPDATE codalraw."SheetsConverted" SET "Exist_IncomeCons"=NULL WHERE "report_ID"=%s"""
        if TypeOf=="IS":
             postgres_insert_query = """
            UPDATE codalraw."SheetsConverted" SET "Exist_Income"=NULL WHERE "report_ID"=%s"""
        if TypeOf=="ISCONSCOMP":
             postgres_insert_query = """
            UPDATE codalraw."SheetsConverted" SET "Exist_IncomeCompCons"=NULL WHERE "report_ID"=%s"""
        if TypeOf=="ISCOMP":
             postgres_insert_query = """
            UPDATE codalraw."SheetsConverted" SET "Exist_IncomeComp"=NULL WHERE "report_ID"=%s"""
...
def get_unconverted():
    try:
        connection = psycopg2.connect(user=db_username,
                                          ...
                                          database=db_database)
        cursor = connection.cursor()
        df = psql.read_sql(""" select * from codalraw."SheetsConverted" inner join
         codalraw."allrawReports" on "report_ID"="TracingNo" where ("Exist_Income"=False or "Exist_IncomeCons"=False
          or "Exist_IncomeComp"=False  or "Exist_IncomeCompCons"=False) order by "SentTime" desc
...
###////////////////////////////////////////////
def get_IncomeStatement_typeI(driver,TypeOfIS,Report_ID):
    # print('Type:'+TypeOfIS)
    results={}
    descs=[]
    select = Select(driver.find_element_by_id('ddlTable'))
    ...
    if TypeOfIS=="IS":
        ...
        time.sleep(2)
    if ('درآمد' not in driver.page_source) &('سهم' not in driver.page_source)&('مالیات' not in driver.page_source):
        print('empttyyyyy')
        # update_Empty(Report_ID,TypeOfIS)
    # UpdateError(driver,Report_ID,TypeOfIS)
    if 'rayanDynamicStatement' in driver.page_source:
        ...
        # print(temp)
        return results
def get_IncomeStatement_typeII_jame(driver,TypeOfIS,Report_ID):
    # print('Type:'+TypeOfIS)
    results={}
    descs=[]
...
def takeCareOFIS(driver,CodalRaw_ID,CodalRaw_links):
    ...
    if('صورت سود و زیان') in allOptions or ('صورت سود و زیان') in allOptions2:
        # print("Income")
        Insert_IncomeStatement(get_IncomeStatement_typeI(driver,"IS",CodalRaw_ID),CodalRaw_ID,CodalRaw_links,'Income')
    if('صورت سود و زیان جامع') in allOptions or ('صورت سود و زیان جامع') in allOptions2:
        # print("IncomeComp")
        Insert_Comprehensive_IncomeStatement(get_IncomeStatement_typeII_jame(driver,"ISCOMP",CodalRaw_ID),CodalRaw_ID,CodalRaw_links,'IncomeComp')
    if('صورت سود و زیان تلفیقی') in allOptions or ('صورت خالص دارایی ها') in allOptions2:
        # print('IncomeCons')
        Insert_IncomeStatement(get_IncomeStatement_typeI(driver,"ISCONS",CodalRaw_ID),CodalRaw_ID,CodalRaw_links,'IncomeCons')
    if('صورت سود و زیان جامع تلفیقی') in allOptions or ('صورت وضعیت مالی تلفیقی') in allOptions2:
        # print('IncomeCompCons')
        Insert_Comprehensive_IncomeStatement(get_IncomeStatement_typeII_jame(driver,"ISCONSCOMP",CodalRaw_ID),CodalRaw_ID,CodalRaw_links,'IncomeCompCons')
###/////
def scrape(df, *, loop):
