...
IC_license=['lblLicenseCode','lblLicenseDate','lblFromAmount','lblToAmount','lblCapitalChangeType',
            'lblSellingPrice','lblSalesCommission','lblPure','lblRightPriorCount','lblPrice']
false_Labels=['lblLicenseCode','lblLicenseDate','lblFromAmount','lblToAmount','lblCapitalChangeType','lblSellingPrice','lblSalesCommission','lblPure','lblRightPriorCount','lblPrice']
...
def InsertError(CodalRaw_ID):
    try:
        ...
        connection.commit()
        log_it("N_66_Error "+str(CodalRaw_ID))
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N66')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۶۶' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertIC_PrioSell(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."IncreaseCapital_SellingPrioritiesResult" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."IncreaseCapital_SellingPrioritiesResult"(
                        firm, report_id, "Correction", 
                        "CorrectionDetails", "LicenseCode", 
                        "LicenseDate", "FromAmount", "ToAmount",
                        "CapitalChangeType", "TotalSellingPrice", "SellCommision",
                        "NetSellingPrice", "PriorCounts", "PriorPrices")
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s,
                        %(lblLicenseCode)s,%(lblLicenseDate)s,
                        %(lblFromAmount)s,%(lblToAmount)s,
                        %(lblCapitalChangeType)s,%(lblSellingPrice)s,
                        %(lblSalesCommission)s,%(lblPure)s,%(lblRightPriorCount)s,
                        %(lblPrice)s
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                ...
                log_it('Failed to Insert RegNotice N66 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
