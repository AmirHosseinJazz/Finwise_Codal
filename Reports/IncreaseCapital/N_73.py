...
IC_Assembly_labels=['txbLastShareCount','txbLastShareValue','txbLastCapital','txbCashIncoming','txbRetaindedEarning',
                    'txbReserves','txbRevaluationSurplus','txbSarfSaham','txbCapitalIncreaseValue','txbIncreasePercent',
                    'txbNewShareCount','txbNewShareValue','txbNewCapital','lblLicenseDesc','lblLastExtraAssembly','lblLastBoardMemberInvitation',
                    'ucExtraAssemblyScheduling1_tdIsRegistered','ucExtraAssemblyScheduling1_spanDesc','lblYearEndToDate','lblLicenseDesc']
false_Labels=['lblYearEndToDate','lblLicenseDesc','lblLastExtraAssembly','lblLastBoardMemberInvitation']
...
def InsertError(CodalRaw_ID):
    try:
        ...
        connection.commit()
        log_it("N_73Error "+str(CodalRaw_ID))
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N73')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۷۳' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertICAResult(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from codalreports."IncreaseCapital_AssemblyResults" where "report_id"=%(report_id)s) THEN
                        INSERT INTO codalreports."IncreaseCapital_AssemblyResults"(
                        firm, report_id, "Correction", "CorrectionDetails",
                        "LastshareCount", "LastshareValue", "LastCapital", 
                        "cashIncoming", "RetainedEarning", "Reserves", 
                        "RevaluationSurplus", "SarfSaham", "CapitalIncreaseValue",
                        "IncreasePercent", "NewshareCount", "NewshareValue", "NewCapital", 
                        "LicenseRef", "LastExtraAssembly", "LastBoardMemberInvitation",
                        "IsRegistered", "RegisteredDesc", "yearEndToDate" 
                        )
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s,
                        %(txbLastShareCount)s,%(txbLastShareValue)s,
                        %(txbLastCapital)s,%(txbCashIncoming)s,%(txbRetaindedEarning)s,%(txbReserves)s,
                        %(txbRevaluationSurplus)s,%(txbSarfSaham)s,%(txbCapitalIncreaseValue)s,%(txbIncreasePercent)s,
                        %(txbNewShareCount)s,%(txbNewShareValue)s,%(txbNewCapital)s,%(lblLicenseDesc)s,%(lblLastExtraAssembly)s,
                        %(lblLastBoardMemberInvitation)s,%(ucExtraAssemblyScheduling1_tdIsRegistered)s,%(ucExtraAssemblyScheduling1_spanDesc)s,%(lblYearEndToDate)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert ICAResult ", error)
                log_it('Failed to Insert ICAResult N73 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
