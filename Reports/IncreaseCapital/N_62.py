...
IC_license=['lblLicenseDate','lblLicenseCode','lblCompanyCapital','lblShareCount','lblShareValue',
           'txbCapitalIncreaseAmountAccept','lblOtherCashIncoming','lblRetaindedEarning',
           'txbCapitalIncreaseTitle' ,'txbFromCapitalIncreasePrice','txbToCapitalIncreasePrice',
            'lblCapitalChangeTypeSecond','txbMultistepYear','lblOtherCashIncomingSecond','lblRetaindedEarningSecond',
           'lblCashIncomingSecond','lblOtherCashIncomingSecond','lblRetaindedEarningSecond','lblReservesSecond',
            'lblOtherReservesSecond','lblRevaluationSurplusAssetsSecond','lblCashIncoming','lblOtherCashIncoming',
            'lblReserves','lblOtherReserves','lblRevaluationSurplusAssets','txbYearEndToDate'
            
           ]
false_Labels=['lblLicenseDate','lblLicenseCode','lblCompanyCapital','lblShareCount','lblShareValue',
           'txbCapitalIncreaseAmountAccept','lblOtherCashIncoming','lblRetaindedEarning',
           'txbCapitalIncreaseTitle' ,'txbFromCapitalIncreasePrice','txbToCapitalIncreasePrice',
            'lblCapitalChangeTypeSecond','txbMultistepYear','lblOtherCashIncomingSecond','lblRetaindedEarningSecond',
           'lblCashIncomingSecond','lblOtherCashIncomingSecond','lblRetaindedEarningSecond','lblReservesSecond',
            'lblOtherReservesSecond','lblRevaluationSurplusAssetsSecond','lblCashIncoming','lblOtherCashIncoming',
            'lblReserves','lblOtherReserves','lblRevaluationSurplusAssets']
...
def InsertError(CodalRaw_ID):
    try:
        ...
        connection.commit()
        log_it("N_62_Error "+str(CodalRaw_ID))
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'N62')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='ن-۶۲' and "HtmlUrl"!='' and "Available"=True  and "TracingNo" not in (select "report_ID" from codalraw."IncreaseCapitalConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertIC_License(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from   codalreports."IncreaseCapital_License" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."IncreaseCapital_License"(
                        firm, report_id, "Correction", "CorrectionDetails",
                        "LicenseDate", "LicenseRef", "PreviousCapital", "ShareCount",
                        "ShareValue", "CapitalIncreaseAmount", "CapitalIncreaseTitle",
                        "CashIncoming_first", "RetainedEarning_first", "CapitalIncreaseStart_Further",
                        "CapitalIncreaseEnd_Further", "CapitalIncreaseDeadline_Further", "CashIncoming_Further",
                        "RetainedEarning_Further", "Reserve_Further", "Reserve2_Further",
                        "RevaluationSurplus_Further", "Reserves_first", "Revaluation_first", 
                        "CapitalChangeType_Further")
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,
                        %(correction)s,%(Details)s,
                        %(lblLicenseDate)s,%(lblLicenseCode)s,%(lblCompanyCapital)s,%(lblShareCount)s,
                        %(lblShareValue)s,%(txbCapitalIncreaseAmountAccept)s,%(txbCapitalIncreaseTitle)s,
                        %(lblOtherCashIncoming)s,%(lblRetaindedEarning)s,%(txbFromCapitalIncreasePrice)s,
                        %(txbToCapitalIncreasePrice)s,%(txbMultistepYear)s,%(lblOtherCashIncomingSecond)s
                        ,%(lblRetaindedEarningSecond)s,%(lblReservesSecond)s,
                        %(lblOtherReserves)s,%(lblRevaluationSurplusAssetsSecond)s,%(lblOtherReserves)s,
                        %(lblRevaluationSurplusAssets)s,%(lblCapitalChangeTypeSecond)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                ...
                log_it('Failed to Insert RegNotice N62-'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    for index,row in DFIC.iterrows():
        insertIC_License(row)        
def RUN(driver):
    df=get_unconverted()
    df=df[~df['title'].str.contains('صدور مجوز افزایش سرمایه')]
    df=df[~df['title'].str.contains('اطلاعیه تکمیلی صدور مجوز افزایش سرمایه')]
