...
IC_license=['lblSeventeenthPlaning','lblSeventeenthOpportunity','lblSeventeenthToDate','lblSeventeenthOpportunity','lblSeventeenthTransparenciesObserveInstructions']

false_Labels=['lblSeventeenthPlaning','lblSeventeenthOpportunity','lblSeventeenthToDate','lblSeventeenthTransparenciesObserveInstructions']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'S13')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='س-۱۳' and "HtmlUrl"!='' and "Available"=True and "TracingNo" not in (select "report_ID" from codalraw."orgClarificationConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def InsertArticle19_5(row):
    try:
        ...
        postgres_insert_query = """
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from   codalreports."OrgClarification_Article19" where "report_id"=%(report_id)s) THEN
                        INSERT INTO  codalreports."OrgClarification_Article19"(
                        firm, report_id, "Subject", "PublishTime",
                        "Reason", "ToDate", "Opportunity")
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,%(Subject)s,%(DateOf)s,
                        %(lblSeventeenthPlaning)s,
                        %(lblSeventeenthToDate)s,%(Opport)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert OrgClar_S5 ", error)
                log_it('Failed to Insert OrgClar_S5 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
def handle_all(driver,df):
    ...
    DFIC['Opport']=DFIC['lblSeventeenthOpportunity']+' '+DFIC['lblSeventeenthTransparenciesObserveInstructions']
    for i in IC_license:
