...
IC_license=['lblTenthPlaning','lblTenthTransparenciesImpotant','lblTenthTransparenciesObserveInstructions']
false_Labels=['lblTenthPlaning','lblTenthTransparenciesImpotant','lblTenthTransparenciesObserveInstructions']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'S6')
...
def get_unconverted():
    try:
        ...
        cursor = connection.cursor()
        df = psql.read_sql("""
        select * FROM codalraw."allrawReports" where "LetterCode"='س-۶' and "HtmlUrl"!='' and "Available"=True and "TracingNo" not in (select "report_ID" from codalraw."orgClarificationConverted")
        """, connection)
        return df
    except (Exception, psycopg2.Error) as error :
            if(connection):
...
def insertIC_Article16_Running(row):
    try:
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from   codalreports."OrgClarification_Article16" where "report_id"=%(report_id)s) THEN
                        ...
                        firm, report_id,"PublishDate", "Subject",
                        "Issue", "Reason", "Instruction")
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,%(DateOf)s,
                        %(Subject)s,%(lblTenthPlaning)s,
                        %(lblTenthTransparenciesObserveInstructions)s,%(lblTenthTransparenciesImpotant)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert OrgClar_S6 ", error)
...
def handle_all(driver,df):
    ...
    for index,row in DFIC.iterrows():
        insertIC_Article16_Running(row)        
def RUN(driver):
    df=get_unconverted()
    if not df.empty:
