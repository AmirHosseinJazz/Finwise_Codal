...
IC_license=['lblThirteenthPlaning','lblThirteenthTransparenciesImpotant','lblThirteenthToDate','lblThirteenthFromDate']

false_Labels=['lblThirteenthPlaning','lblThirteenthTransparenciesImpotant','lblThirteenthToDate','lblThirteenthFromDate']
...
def log_it(text):
    try:
        ...
        record_to_insert = (str(datetime.datetime.now()),text,'S7')
...
def get_unconverted():
    try:
        ...
        select * FROM codalraw."allrawReports" where "LetterCode"='س-۷' and "HtmlUrl"!='' and "Available"=True and "TracingNo" not in (select "report_ID" from codalraw."orgClarificationConverted")
...
def insertArticle_19_1(row):
    try:
            ...
            $$
                BEGIN
                    IF NOT EXISTS (select from   codalreports."OrgClarification_Article19" where "report_id"=%(report_id)s) THEN
                        ...
                        firm, report_id, "Subject", "PublishTime", "Issue", "Reason",
                        "FromDate", "ToDate")
                        VALUES ( (select "ID" from "Publishers" where "persianName"=%(Nemad)s), %(report_id)s,
                        %(Subject)s,%(DateOf)s,%(lblThirteenthPlaning)s,
                        %(lblThirteenthTransparenciesImpotant)s,%(lblThirteenthToDate)s,%(lblThirteenthToDate)s
                        );
                    END IF;
                END
            $$ 
        ...
        print(str(row['report_id'])+'  '+'--Done')
            ...
            if(connection):
                print("Failed to Insert OrgClar_S7 ", error)
                log_it('Failed to Insert OrgClar_S7 -'+str(row['report_id']))
    finally:
        if(connection):
            cursor.close()
            connection.close()   
