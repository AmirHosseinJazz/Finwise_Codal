...
#///////////
GoForItemsBS=['موجودی نقد','تسهیلات مالی بلندمدت','سرمایه','جمع بدهی‌ها','جمع حقوق صاحبان سهام','جمع دارایی\u200cها','جمع بدهی\u200cهای جاری','جمع دارایی\u200cهای جاری','پرداختنیهای تجاری و سایر پرداختنیها','سرمایه\u200cگذاری\u200c\u200cهای کوتاه\u200cمدت']
GoForItemsIS=['سود (زیان) عملیاتی','سایر درآمدها و هزینه‌های غیرعملیاتی','سایر درآمدها و هزینه‌های غیرعملیاتی- اقلام متفرقه','سود (زیان) عملیات در حال تداوم قبل از مالیات','مالیات','درآمدهای عملیاتی','سود (زیان) خالص']
GoForItemsCF=['جریانهای نقدی حاصل از فعالیتهای عملیاتی','جریان خالص ورود(خروج) وجه نقد ناشی از فعالیت‌های عملیاتی-عادی','جریان خالص ورود(خروج) وجه نقد ناشی از فعالیت‌های عملیاتی - استثنایی','وجوه دریافتی بابت فروش دارایی‌های ثابت مشهود','وجوه پرداختی بابت خرید دارایی‌های ثابت مشهود']
#/////////
DiscountRate=0.45
...
async def SortOneOut(ID,session):
    ...
    try:
        ...
        # try:
        #     head = {'Accept-Profile':'statement'}
        #     async with session.get('http://130.185.74.40:3000/rpc/isall?a='+str(ID),timeout=15,headers=head) as resp:
        #         wholeFile=await resp.text()
        #         if wholeFile is None or wholeFile=='':
        #             raise Exception('No IS Data')
        # except Exception as E:
        #     print('Error: '+E)
        try:
            connection = psycopg2.connect(user=db_username,
            ...
            cursor = connection.cursor()
            ISAll = psql.read_sql("""
            SELECT * from statement.isall('"""+str(ID)+"""')""", connection)
        except Exception as E:
            print('Error: '+E)
        # print(wholeFile)
        # ISAll=pd.DataFrame(json.loads(wholeFile))
        if ISAll.empty:
            raise Exception('No IS Data')
    except Exception as E2:
        # print(E2)
    ...
    try:
        ...
        # try:
        #     head = {'Accept-Profile':'statement'}
        #     async with session.get('http://130.185.74.40:3000/rpc/cfall?a='+str(ID),timeout=15,headers=head) as resp:
        #         wholeFile=await resp.text()
        #         if wholeFile is None or wholeFile=='':
        ...
        try:
            ...
            cursor = connection.cursor()
            CFAll = psql.read_sql("""
            SELECT * from statement.cfall('"""+str(ID)+"""')""", connection)
        except Exception as E:
            print('Error: '+E)
        # print(wholeFile)
        # CFAll=pd.DataFrame(json.loads(wholeFile))
        if CFAll.empty:
            raise Exception('No CF Data')
    except Exception as E2:
        # print(E2)
    ...
    try:
        print('Fetching GetAll Quarterly')
        # wholeFile=''
        # try:
        #     head = {'Accept-Profile':'statement'}
        #     async with session.get('http://130.185.74.40:3000/View_RatioItems?firm=eq.'+str(ID),timeout=15,headers=head) as resp:
        #         wholeFile=await resp.text()
        #         if wholeFile is None or wholeFile=='':
        #             raise Exception('No Q Data')
        # except Exception as E:
        #     print('Error: '+E)
        try:
            connection = psycopg2.connect(user=db_username,
                                          ...
                                          database=db_database)
            cursor = connection.cursor()
            Quarterly = psql.read_sql("""
            SELECT * FROM statement."View_RatioItems" where firm="""+str(ID), connection)
        except Exception as E:
            print('Error: '+E)
        # print(wholeFile)
        # Quarterly=pd.DataFrame(json.loads(wholeFile))
        if Quarterly.empty:
            raise Exception('No Q Data')
    except Exception as E2:
        # print(E2)
    ...
    NeededIS=[]
    NeededCF=[]
    BS=BSAll[(BSAll['period']==12)&(BSAll['aggregated']==False)]
    IS=ISAll[(ISAll['period']==12)&(ISAll['aggregated']==False)]
    CF=CFAll[(CFAll['period']==12)&(CFAll['Aggregated']==False)]
    CF.dropna(subset=['thisPeriod'],inplace=True)
    DPS=DPSALL.sort_values(by=['AssemblyDate'],ascending=False)
    DPS['year']=DPS['AssemblyDate'].apply(lambda x: int(str(x).split('/')[0]))
    # print(DPS)
    for it in GoForItemsBS:
        for index,row in BS[BS['Translated']==it].iterrows():
            Date=(row['toDate'].split('/')[0])
    ...
    for it in GoForItemsIS:
        for index,row in IS[(IS['Translated']==it)].iterrows():
            Date=(row['toDate'].split('/')[0])
            Value=row['thisPeriod']
            Item=row['Translated']
    ...
    for it in GoForItemsCF:
        for index,row in CF[(CF['Translated']==it)].iterrows():
            Date=(row['toDate'].split('/')[0])
            Value=row['thisPeriod']
            Item=row['Translated']
    ...
    #
    Quarterly.sort_values(by=['year','quarter'],ascending=[False,False],inplace=True)
    for tDate in Quarterly.toDate.unique():
        try:
            r=(Quarterly[(Quarterly['translatedName']=='سود (زیان) خالص')&(Quarterly['toDate']==tDate)]).iloc[0]
            try:
                r2=(Quarterly[(Quarterly['translatedName']=='سایر درآمدها و هزینه\u200cهای غیرعملیاتی- اقلام متفرقه')&(Quarterly['toDate']==tDate)]).iloc[0]
                Quarterly=Quarterly.append({'translatedName':'NRI','engName':'ISItems','value':r.value-r2.value,'quarter':r['quarter']
                ,'firm':r['firm'],'toDate':r['toDate'],'aggregated':r['aggregated'],'year':r['year']},ignore_index=True)
            except:
                Quarterly=Quarterly.append({'translatedName':'NRI','engName':'ISItems','value':r.value,'quarter':r['quarter']
                ,'firm':r['firm'],'toDate':r['toDate'],'aggregated':r['aggregated'],'year':r['year']},ignore_index=True)
        except:
            print(r)
            break
    Quarterly.sort_values(by=['year','quarter'],ascending=[False,False],inplace=True)
    # #////////
    for name in Quarterly[(Quarterly['engName']=='CFItems')| (Quarterly['engName']=='ISItems')].translatedName.unique().tolist():
        c=False
        QDFTemp=Quarterly[Quarterly['translatedName']==name]
        QDFTemp.sort_values(by=['toDate'],ascending=False,inplace=True)
