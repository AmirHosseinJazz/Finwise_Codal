...
async def SortOneOut(ID,session):
    ...
    try:
        ...
        for UniqueToDate in Quarterly.toDate.unique().tolist():
            try:
                ...
                if len(Total[(Total['Item']=='بهای تمام \u200cشده درآمدهای عملیاتی')&(Total['date']==str(LastYear))])>0:
                    COGST1=Total[(Total['Item']=='بهای تمام \u200cشده درآمدهای عملیاتی')&(Total['date']==str(LastYear))].iloc[0].value
                else:
                    if len(Total[(Total['Item']=='بهای تمام شده کالای فروش رفته')&(Total['date']==str(LastYear))])>0:
                        COGST1=Total[(Total['Item']=='بهای تمام شده کالای فروش رفته')&(Total['date']==str(LastYear))].iloc[0].value
                    else:
                        if len(Total[(Total['Item']=='بهای تمام شده فروش')&(Total['date']==str(LastYear))])>0:
                            COGST1=Total[(Total['Item']=='بهای تمام شده فروش')&(Total['date']==str(LastYear))].iloc[0].value
                #
                if len(Quarterly[(Quarterly['translatedName']=='دریافتنیهای تجاری و سایر دریافتنی ها')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(year))])>0:
                    ...
                    if len(Quarterly[(Quarterly['translatedName']=='دریافتنی‌‌های غیرتجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(year))])>0:
                        RecievablesShortTerm_NOW=Quarterly[(Quarterly['translatedName']=='دریافتنی‌‌های غیرتجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(year))].iloc[0].value
                        RecievablesShortTerm_NOW+=Quarterly[(Quarterly['translatedName']=='دریافتنی‌‌های تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(year))].iloc[0].value
                    else:
                        if len(Quarterly[(Quarterly['translatedName']=='حصه جاری حساب‌ها و اسناد دریافتنی تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(year))])>0:
                            RecievablesShortTerm_NOW+=Quarterly[(Quarterly['translatedName']=='حصه جاری حساب‌ها و اسناد دریافتنی تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(year))].iloc[0].value
                
                
                if len(Quarterly[(Quarterly['translatedName']=='دریافتنیهای تجاری و سایر دریافتنی ها')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))])>0:
                    ...
                    if len(Quarterly[(Quarterly['translatedName']=='دریافتنی‌‌های غیرتجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))])>0:
                        RecievablesShortTerm_T0=Quarterly[(Quarterly['translatedName']=='دریافتنی‌‌های غیرتجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))].iloc[0].value
                        RecievablesShortTerm_T0+=Quarterly[(Quarterly['translatedName']=='دریافتنی‌‌های تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))].iloc[0].value
                    else:
                        if len(Quarterly[(Quarterly['translatedName']=='حصه جاری حساب‌ها و اسناد دریافتنی تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))])>0:
                            RecievablesShortTerm_T0+=Quarterly[(Quarterly['translatedName']=='حصه جاری حساب‌ها و اسناد دریافتنی تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))].iloc[0].value
                ...
                if len(Quarterly[(Quarterly['translatedName']=='پرداختنیهای تجاری و سایر پرداختنیها')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))])>0:
                    CommercialPayables_T1=Quarterly[(Quarterly['translatedName']=='پرداختنیهای تجاری و سایر پرداختنیها')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))].iloc[0].value
                else:
                    if len(Quarterly[(Quarterly['translatedName']=='پرداختنی‌های تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))])>0:
                        CommercialPayables_T1=Quarterly[(Quarterly['translatedName']=='پرداختنی‌های تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))].iloc[0].value
                        CommercialPayables_T1+=Quarterly[(Quarterly['translatedName']=='پرداختنی‌های غیرتجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))].iloc[0].value
                    else:
                        if len(Quarterly[(Quarterly['translatedName']=='حصه جاری حساب‌ها و اسناد دریافتنی تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))])>0:
                            CommercialPayables_T1+=Quarterly[(Quarterly['translatedName']=='حصه جاری حساب‌ها و اسناد دریافتنی تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(LastYear))].iloc[0].value
                
                
                if len(Quarterly[(Quarterly['translatedName']=='پرداختنیهای تجاری و سایر پرداختنیها')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(YearBeforeLastYear))])>0:
                    CommercialPayables_T0=Quarterly[(Quarterly['translatedName']=='پرداختنیهای تجاری و سایر پرداختنیها')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(YearBeforeLastYear))].iloc[0].value
                else:
                    if len(Quarterly[(Quarterly['translatedName']=='پرداختنی‌های تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(YearBeforeLastYear))])>0:
                        CommercialPayables_T0=Quarterly[(Quarterly['translatedName']=='پرداختنی‌های تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(YearBeforeLastYear))].iloc[0].value
                        CommercialPayables_T0+=Quarterly[(Quarterly['translatedName']=='پرداختنی‌های غیرتجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(YearBeforeLastYear))].iloc[0].value
                    else:
                        if len(Quarterly[(Quarterly['translatedName']=='حساب‌ها و اسناد پرداختنی تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(YearBeforeLastYear))])>0:
                            CommercialPayables_T0+=Quarterly[(Quarterly['translatedName']=='حساب‌ها و اسناد پرداختنی تجاری')&(Quarterly['quarter']==quarter)&(Quarterly['year']==str(YearBeforeLastYear))].iloc[0].value
                ######//////
    ...
    try:
        ...
        CREATE TEMPORARY TABLE _data3(
            firm bigint NOT NULL,
            quarter character varying COLLATE pg_catalog."default" NOT NULL,
            year character varying COLLATE pg_catalog."default",
            "toDate" character varying COLLATE pg_catalog."default" NOT NULL,
            "Component" character varying COLLATE pg_catalog."default" NOT NULL,
            "ComponentValue" double precision NOT NULL,
            "CalculatedOn" character varying COLLATE pg_catalog."default" NOT NULL,
            CONSTRAINT "ValuationRatiosComponents_pkey" PRIMARY KEY (firm,quarter, "toDate", "Component", "ComponentValue", "CalculatedOn")
            )''')
        for k in Alltuples:
            # print(k)
            result = await con.copy_records_to_table('_data3', records=k)
        ...
        SELECT * FROM _data3
        ON CONFLICT (firm,quarter, "toDate", "Component", "ComponentValue", "CalculatedOn")
        DO NOTHING
